...
public class AtomImpl implements Atom {
	...
	Atom a = (Atom)atom;
	boolean result = predicateID.equals( a.getPredicateID()) || predicateLabel.equals(a.getPredicateID());

	if (result)
	    if (a.getCountVariables() != getCountVariables())
		result = false;
	    else
		for (int i = 0; i < getCountVariables(); i++)
		    if (!a.getVariables().get(i).equals(getVariables().get(i))){
			result = false;
			break;
		    }
	return result;
