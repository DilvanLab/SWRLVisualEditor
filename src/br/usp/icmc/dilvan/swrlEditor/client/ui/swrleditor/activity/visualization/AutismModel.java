...
public class AutismModel {
    protected Rule rule;
    private final TYPE_VIEW typeView;

    // List of errors
    ArrayList<String> errors = new ArrayList<>();

    // The main atom
    ArrayList<Atom> mainAnt = new ArrayList<>();
    ArrayList<Atom> mainCons = new ArrayList<>();

    // List of every builtins
    ArrayList<Atom> builtins = new ArrayList<>();
    ...
    HashMap<String, ArrayList<Atom>> properties1 = new HashMap<>();
    HashMap<String, ArrayList<Atom>> properties2 = new HashMap<>();

    public AutismModel(Rule r, TYPE_VIEW typeViewInOptions){
	rule = r;
	typeView = typeViewInOptions;
	...
	HashMap<String, BuiltinMap> dict = new HashMap<>();
	// add
	...
	if(matches.size() == 1 && !inBuiltin(matches.get(0).getVariables().get(1)) ){
		...
		} else if(blt.containsKey(a.getPredicateLabel().toLowerCase())){
